...
class ConsumerConnector(BaseConnector):
    ...
    def poll(self):
        return self.channel.basic_get(queue=self.consumer_queue)

    def pull(self, callback: Callable) -> None:
        logger.info(f"Starting Pull on Queue: {self.consumer_queue}")
        self.channel.basic_get(queue=self.consumer_queue, on_message_callback=callback)
...
class ProducerConnector(BaseConnector):
    ...
    def connect(self) -> None:
        ...
        """
        super().connect()
        self.channel.exchange_declare(exchange=self.publish_exchange, exchange_type="direct")
...
if __name__ == '__main__':
    # RABBITMQ_CONSUMER_CONFIG = {"rabbitmq_host": "***",
    #                             "rabbitmq_port": 5672,
    #                             "rabbitmq_virtual_host": "***",
    #                             "rabbitmq_user": "crawler",
    #                             "rabbitmq_pass": "***",
    #                             "consumer_queue": 'queue-test',
    #                             "retry_on_connection_error": True}
    # consumer = ConsumerConnector(**RABBITMQ_CONSUMER_CONFIG)
    # consumer.connect()
    # while 1:
    #     method_frame, header_frame, body = consumer.poll()
    #     if method_frame:
    #         consumer.channel.basic_ack(method_frame.delivery_tag)
    # breakpoint()
    RABBITMQ_CONFIG = {
        "producer_queue_bindings": [{"routing_key": 'queue-test', "bound_queue": 'queue-test'}],
        "rabbitmq_host": "****",
        "rabbitmq_port": 5672,
        "rabbitmq_virtual_host": "crawler",
        "rabbitmq_user": "crawler",
        "rabbitmq_pass": "****",
        "publish_exchange": 'tasks',
        "retry_on_connection_error": True}
    producer = ProducerConnector(**RABBITMQ_CONFIG)
    producer.connect()
    producer.post('queue-test', {"name": "aa"})
